...
\mathrm{cv}_i=\frac{y_i-\widehat{y}_i}{1-l_i},\quad i=1,\dots,n
...
\begin{mdquote}
\textbf{图 4.13}：两组直方图分别展示 (64,6) 极大极小设计与 (64,6) 最大投影设计样本两两间距的分布；贝塔分布密度函数定义域缩放至 $[0,\sqrt6]$ 区间
\end{mdquote}
...
\begin{mdquote}
\textbf{图 5.1}：7 个随机采样点（左）与最优采样点（右）的经验分布函数
\end{mdquote}
...
\mathcal{E}(F,G)\equiv2\mathrm{E}\|\boldsymbol{X}-\boldsymbol{Y}\|-\mathrm{E}\|\boldsymbol{X}-\boldsymbol{X}'\|-\mathrm{E}\|\boldsymbol{Y}-\boldsymbol{Y}'\|\tag{5.9}
\end{equation*}
\end{mdquote}
...
\argmin_{D}\mathcal{E}(F,F_n)=\argmin_{D}\left\{\frac{2}{n}\sum_{i=1}^{n}\mathrm{E}\|\boldsymbol{x}_i-\boldsymbol{X}\|-\frac{1}{n^2}\sum_{i=1}^{n}\sum_{j=1}^{n}\|\boldsymbol{x}_i-\boldsymbol{x}_j\|\right\}\tag{5.11}

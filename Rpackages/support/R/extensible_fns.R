sp_seq=function(D,nseq,ini=NA,num.rep=1,dist.str=NA,dist.param=vector("list",p),dist.samp=NA,scale.flg=TRUE,bd=NA,num.subsamp=ifelse(any(is.na(dist.samp)),max(10000,10*(nseq+nrow(D))),min(10000,nrow(dist.samp))),iter.max=max(200,iter.min),iter.min=50,tol=1e-10,par.flg=TRUE){
  cur=D
  p=ncol(cur)
  ...
  for(jj in 1:num.rep){
    if(std.flg){
      ...
      des=sp_seq_cpp(cur,nseq,ini,dist.ind,dist.param,dist.samp,FALSE,bd,num.subsamp,iter.max,iter.min,tol,num.cores)
    }else{
      if(scale.flg==T){
      ...
      des=sp_seq_cpp(cur,nseq,ini,dist.ind,dist.param,dist.samp,TRUE,bd,num.subsamp,iter.max,iter.min,tol,num.cores)
      if(scale.flg==T){
